...
if 'paper_positions' not in st.session_state:
    st.session_state.paper_positions = [] # 模拟持仓
...
class MacroAgent:
    """5. Macro Agent: 美联储、CPI、ETF 资金流分析"""
    @staticmethod
    def run():
        return {"score": 65, "dxy": "103.85 (-0.2%)", "etf_inflow": "+$182.5M (连续净流入)"}
...
class ReviewAgent:
    """9. Review Agent: 错题集归因与经验沉淀"""
    @staticmethod
    def run():
        return {"score": 90, "advice": "上次模拟交易严格按止损离场，避开了 5% 深度回调，止盈建议提高至 2x ATR。"}
...
# ==========================================
# 7. 展示与交互层 (Presentation Layer - 6大 Tab)
# ==========================================
...
# ------------------------------------------
# TAB 2: 策略与模拟交易 (Strategy & Paper Trading)
# ------------------------------------------
with tab2:
    ...
    with pt_col2:
        ...
        if st.session_state.paper_positions:
            for i, pos in enumerate(st.session_state.paper_positions):
                ...
                st.info(f"**{pos['id']}** | {pos['symbol']} | {pos['side']} | 开仓价: {format_price(pos['entry_price'])} | 保证金: ${pos['margin']} | 未实现盈亏: **${pnl:+.2f} USDT**")
                if st.button(f"一键平仓 {pos['id']}", key=f"close_{i}"):
                    st.session_state.paper_balance += pnl
                    st.session_state.paper_positions.pop(i)
                    st.success(f"平仓结算完成，账户总余额更新为: ${st.session_state.paper_balance:.2f} USDT")
                    st.rerun()
        else:
            st.write("暂无活跃模拟持仓。")

# ------------------------------------------
# TAB 3: 风险中心 (Risk Center & 10U Guard)
# ------------------------------------------
with tab3:
...
with tab6:
    ...
    st.divider()
    st.subheader("📲 Telegram Bot 自动化报告推送")
